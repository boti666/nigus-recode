...
public:
	...
	__forceinline bool GameSetupBonesLAWL(matrix3x4_t* out, int max, int mask, float time) {
...
#define FIRSTPERSON_TO_THIRDPERSON_VERTICAL_TOLERANCE_MAX 10.0f
	...
	__forceinline bool SetupBones(matrix3x4_t* bonetoworld, int maxbones, int bonemask, float currenttime) {
		...
		const auto framecount = g_csgo.m_globals->m_frame;
		const auto curtime_backup = g_csgo.m_globals->m_curtime;
		const auto realtime_backup = g_csgo.m_globals->m_realtime;
		const auto frametime = g_csgo.m_globals->m_frametime;
		const auto absoluteframetime = g_csgo.m_globals->m_abs_frametime;
		const auto tickcount = g_csgo.m_globals->m_tick_count;
		const auto interpolation_amount = g_csgo.m_globals->m_interp_amt;

		float time = currenttime;
		int ticks = game::TIME_TO_TICKS(time);
		...
		*boneSnapshot2 = 0.0f;
		...
		g_csgo.m_allow_setupbones[index() - 1] = true;
		auto bone_result = GameSetupBonesLAWL(bonetoworld, maxbones, bonemask, currenttime);
		g_csgo.m_allow_setupbones[index() - 1] = false;
