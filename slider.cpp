...
void Slider::draw() {
	...
	// bar.
	if (g_menu.main.m_mode == 0) {
		render::gradient(p.x + SLIDER_X_OFFSET + 1, p.y + m_offset + 1, m_fill - 2, SLIDER_HEIGHT - 2, color, color2);
	}
	else {
	...
	if (m_health && m_value > 100.f) {
		ss << std::fixed << std::setprecision(m_precision) << m_suffix << "+" << (m_value - 100.f);
	}
	else
