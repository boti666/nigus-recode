...
void Shots::OnShotFire(Player* target, float damage, LagRecord* record, vec3_t end_pos) {
	...
	ShotRecord shot;
	shot.m_i = target ? target->index() : -1;
	shot.m_target = target ? target : nullptr;
	shot.m_record = record ? record : nullptr;
...
void Shots::OnImpact(IGameEvent* evt) {
	...
	static ConVar* sv_showimpacts_time = g_csgo.m_cvar->FindVar(HASH("sv_showimpacts_time"));

	if (g_menu.main.visuals.impacts.get())
		g_csgo.m_debug_overlay->AddBoxOverlay(pos, { -2, -2, -2 }, { 2, 2, 2 }, { 0, 0, 0 }, 0, 0, 255, 125,
	...
	g_csgo.m_engine_trace->TraceRay(Ray(match.shot->m_pos, pos), MASK_SHOT, (ITraceFilter*)&filter, &trace[0]);

	m_impact = trace[0].m_endpos;

	if (!shot->m_target || !shot->m_record)
		return;
	...
	if (!shot->m_record || shot->m_record->m_player != target)
		return;
	...
	g_csgo.m_engine_trace->ClipRayToEntity(Ray(start, end), MASK_SHOT, target, &trace[1]);

	ImpactRecord* cur_impact = &m_impacts.front();

	cur_impact->m_spread = !trace[1].m_entity || trace[1].m_entity->index() != target->index();
...
void Shots::OnHurt(IGameEvent* evt) {
	...
	g_visuals.m_hitmarkers.emplace_front(m_impact, game::TICKS_TO_TIME(impact->m_tick));
