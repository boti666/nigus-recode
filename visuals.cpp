...
void Visuals::Hitmarker() {
	...
	static vec2_t screen_pos[] = { { 0,0 },  { 0,0 } };
	static bool render_world = false;

	int alpha = 0;

	for (int i = 0; i < m_hitmarkers.size();) {
		vec3_t pos = m_hitmarkers.at(i).first;
		float time = m_hitmarkers.at(i).second;

		float complete = fabs(g_csgo.m_globals->m_curtime - time);
		...
		alpha = std::max(std::min((int)((1.f - complete) * 255), 255), 0);

		screen_pos[0] = { g_cl.m_width / 2.f, g_cl.m_height / 2.f };
		render_world = render::WorldToScreen(pos, screen_pos[1]);
	...
	for (int i = 0; i <= 1;) {
		if (!g_menu.main.misc.hitmarker.get(i)) {
			++i;
			continue;
		}
		...
		render::line(screen_pos[i].x - line, screen_pos[i].y - line, screen_pos[i].x - (line / 2.f), screen_pos[i].y - (line / 2.f), { 255, 255, 255, alpha });
		render::line(screen_pos[i].x - line, screen_pos[i].y + line, screen_pos[i].x - (line / 2.f), screen_pos[i].y + (line / 2.f), { 255, 255, 255, alpha });
		render::line(screen_pos[i].x + line, screen_pos[i].y + line, screen_pos[i].x + (line / 2.f), screen_pos[i].y + (line / 2.f), { 255, 255, 255, alpha });
		render::line(screen_pos[i].x + line, screen_pos[i].y - line, screen_pos[i].x + (line / 2.f), screen_pos[i].y - (line / 2.f), { 255, 255, 255, alpha });
		++i;
...
void Visuals::think() {
	...
	if (fabs(g_csgo.m_globals->m_curtime - last_render) >= 0.01f) {
		g_grenades_pred.get_local_data().draw();
		last_render = g_csgo.m_globals->m_curtime;
	}
}
...
void Visuals::StatusIndicators() {
	...
	if (g_menu.main.visuals.indicators.get(0)) {
		...
		Indicator_t ind{ };
		ind.color = g_menu.main.antiaim.desync_foot_yaw.get() ? Color(255, 255, 60, 255) : (change > 35.f ? 0xff15c27b : Color(255, 0, 0, 255));
		ind.text = XOR("LBY");
	...
	if (g_menu.main.visuals.indicators.get(1)) {
		if (g_cl.m_local->m_vecVelocity().length_2d() > 270.f || g_cl.m_lagcomp) {
			Indicator_t ind{ };
			ind.color = g_cl.m_lagcomp ? 0xff15c27b : Color(255, 0, 0, 255);
			ind.text = XOR("LC");
	...
	if  (g_menu.main.visuals.indicators.get(6) && g_input.GetKeyState(g_menu.main.movement.autopeek.get())) {
		Indicator_t ind{ };
	...
	if (g_menu.main.visuals.indicators.get(2) && (g_aimbot.m_fake_latency || g_menu.main.misc.force_fake_latency.get())) {
		Indicator_t ind{ };
		ind.color = Color(255, 255, 255, 255);
		ind.text = XOR("PING");
	...
	if (g_menu.main.visuals.indicators.get(5) && g_aimbot.m_headglitch) {
		Indicator_t ind{ };
		ind.color = Color(255, 255, 255, 255);
		ind.text = XOR("HG");
	...
	if (g_menu.main.visuals.manual_antiaim.get()) {
		...
		if (g_movement.m_autopeek && !g_movement.m_reached_target)
			color = g_menu.main.movement.autopeek_color.get();

		switch (g_menu.main.visuals.manual_antiaim.get()) {
		case 1: {
			if (g_hvh.m_left) {
				render::triangle(vec2_t(x - (offset + size * 2), y), vec2_t(x - offset, y - size), vec2_t(x - offset, y + size), color, true);

				if (g_hvh.m_fake_flick)
					render::triangle(vec2_t(x + (offset + size * 2), y), vec2_t(x + offset, y + size), vec2_t(x + offset, y - size), g_menu.main.antiaim.fake_flick_color.get(), true);
			}
			...
			if (g_hvh.m_back) {
				render::triangle(vec2_t(x, y + (offset + size * 2)), vec2_t(x - size, y + offset), vec2_t(x + size, y + offset), color, true);

				if (g_hvh.m_fake_flick)
					render::triangle(vec2_t(x, y - (offset + size * 2)), vec2_t(x + size, y - offset), vec2_t(x - size, y - offset), g_menu.main.antiaim.fake_flick_color.get(), true);
			}
	...
	//if (g_menu.main.visuals.crosshair_ind.get()) {
		//int add = 10;

		//if (g_menu.main.visuals.indicators.get(3) && g_aimbot.m_override) {
		//	std::string text = "dmg: " + std::to_string((int)g_menu.main.aimbot.dmg_override_amount.get());

		//	auto size = render::override.size(text);
	
		//	render::override.string(x + size.m_width + 10, y + add - size.m_height * 0.5, { 255, 255, 255, 255 }, text, render::ALIGN_RIGHT);

		//	add += 18;
		//}
	//	if (g_menu.main.visuals.indicators.get(5)) {
		//	std::string text = "lag: " + std::to_string(g_cl.m_lag);
		...
		if (model) {
			...
			if (name.find(XOR("flashbang")) != std::string::npos)
				...
				if (g_menu.main.visuals.proj_range.get(0))
					render::sphere(origin, 50.f, 5.f, 1.f, g_menu.main.visuals.proj_range_color.get());
	...
	if (!local)// draw flags.
	{
		...
		for (auto it = items.begin(); it != items.end(); ++it) {
			...
			if (*it == 6 && !records.empty()) {
				...
				flags.push_back({ "LBY", (current->m_mode == Resolver::Modes::RESOLVE_FLICK ||
					current->m_mode == Resolver::Modes::RESOLVE_WALK ||
					current->m_mode == Resolver::Modes::RESOLVE_PREFLICK) ? Color(150, 200, 60, low_alpha) : Color(255, 0, 0, low_alpha) });
			}
...
void Visuals::DrawHitboxMatrix(LagRecord* record, Color col, float time, int hitbox) {
	const model_t* model;
	studiohdr_t* hdr;
...
void Visuals::DebugAimbotPoints(Player* player) {
	...
	// get bone matrix.
	BoneArray* matrix = cache->m_pCachedBones;
	if (!matrix)
		return;

	data->SetupHitboxes(&data->m_records[0], false);
	...
	for (const auto& it : data->m_hitboxes) {
		...
		if (!data->SetupHitboxPoints(player->m_angEyeAngles().x, matrix, it.m_index, p1))
			continue;
	...
	for (auto& p : p2) {
		...
		//if (render::WorldToScreen(p, screen))
			//render::rect_filled(screen.x, screen.y, 2, 2, g_menu.main.aimbot.point_color.get());
	}
}